    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex bg-stone-100 dark:bg-stone-800 rounded-lg p-1">
          <button onClick={() => setCategoryFilter('all')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${categoryFilter === 'all' ? 'bg-white dark:bg-stone-700 text-stone-900 dark:text-stone-100 shadow-sm' : 'text-stone-600 dark:text-stone-400'}`}>{t('allCategories')}</button>
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setCategoryFilter(cat.id)} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${categoryFilter === cat.id ? 'bg-white dark:bg-stone-700 text-stone-900 dark:text-stone-100 shadow-sm' : 'text-stone-600 dark:text-stone-400'}`}>{isRTL ? cat.labelAr : cat.labelEn}</button>
          ))}
        </div>
      </div>
      ...
      {filteredInventory.length === 0 ? (
        ...
        <div className="space-y-8">
          {groupedInventory.map(({ category, items }) => (
            <div key={category.id}>
              <h2 className="text-lg font-semibold text-stone-900 dark:text-stone-100 mb-4">{isRTL ? category.labelAr : category.labelEn}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-stretch">
                {items.map((item) => <InventoryItemCard key={item.id} item={item} category={category} onEdit={() => onEdit?.(item.id)} onDelete={() => onDelete?.(item.id)} onAdjustQuantity={(delta) => onAdjustQuantity?.(item.id, delta)} />)}
              </div>
            </div>
          ))}
        </div>
