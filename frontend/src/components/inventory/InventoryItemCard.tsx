    ...
    >
      ...
      <div className="flex items-start justify-between p-3 pb-0">
        ...
        <div className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
          <IconButton
            className="h-7 w-7 text-stone-400 hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-amber-900/20"
          ...
          />
          <IconButton
            className="h-7 w-7 text-stone-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20"
      ...
      <div className="flex flex-1 flex-col items-center px-3 py-2 text-center">
        ...
        <div className="flex items-baseline justify-center gap-1">
          <span className="font-semibold text-amber-600 text-base dark:text-amber-500">{formatCurrency(item.price)}</span>
          <span className="font-semibold text-stone-400 text-[10px] uppercase">{t("egp")}</span>
        </div>
      </div>

      {/* 3. Footer: Unified Stock Control */}
      <div className="mt-auto flex items-center justify-between rounded-b-xl border-stone-100 border-t bg-stone-50/50 p-2 dark:border-stone-800 dark:bg-stone-900/50">
        {/* Stock Indicator */}
        <div className="flex flex-col">
          <div className="flex items-center gap-1.5">
            <span className={`font-semibold font-mono text-lg ${statusColors[status]}`}>
              {item.quantity}
            </span>
            {status !== "in-stock" && (
              <AlertCircle
                className={`h-3.5 w-3.5 ${status === "out-of-stock" ? "text-red-500" : "text-amber-500"}`}
              />
            )}
          </div>
          <span className="font-semibold text-stone-400 text-[10px] uppercase tracking-tight">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-stone-200 bg-white p-1 shadow-sm dark:border-stone-700 dark:bg-stone-800">
          <IconButton
            className="h-6 w-6 bg-stone-100 text-stone-600 hover:bg-red-100 hover:text-red-600 dark:bg-stone-700 dark:text-stone-300"
          ...
          />
          <IconButton
            className="h-6 w-6 bg-stone-100 text-stone-600 hover:bg-emerald-100 hover:text-emerald-600 dark:bg-stone-700 dark:text-stone-300"
