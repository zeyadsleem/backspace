          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                {outOfStockItems.length} {language === 'ar' ? 'منتج نفذ' : 'item(s) need restocking'}
              </p>
              <div className="flex flex-wrap gap-2 mt-3">
                {outOfStockItems.slice(0, 5).map((item) => (
                  <button
                    ...
                    className="inline-flex items-center px-3 py-1.5 bg-white dark:bg-stone-900 
                ...
                {outOfStockItems.length > 5 && (
                  <span className="text-sm text-red-600 dark:text-red-400 self-center">
                    +{outOfStockItems.length - 5} {language === 'ar' ? 'أخرى' : 'more'}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-amber-800 dark:text-amber-200">
                {language === 'ar' ? 'مخزون منخفض' : 'Low Stock Warning'}
              </h3>
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                {lowStockItems.length} {language === 'ar' ? 'منتج أقل من الحد الأدنى' : 'item(s) below minimum level'}
              </p>
              <div className="flex flex-wrap gap-2 mt-3">
                ...
                {lowStockItems.length > 5 && (
                  <span className="text-sm text-amber-600 dark:text-amber-400 self-center">
                    +{lowStockItems.length - 5} {language === 'ar' ? 'أخرى' : 'more'}
                  </span>
                )}
              </div>
            </div>
