...
export function InvoiceRow({ invoice, onView, onRecordPayment }: InvoiceRowProps) {
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4 px-4 py-3 hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors ${isRTL ? 'text-end' : 'text-start'}`}>
      <div className="col-span-1 md:col-span-2 flex items-center"><span className="font-mono text-[11px] font-medium text-stone-500 dark:text-stone-400 uppercase tracking-tight">{invoice.invoiceNumber}</span></div>
      ...
      <div className="col-span-1 md:col-span-2 flex items-center md:justify-center">
        <div className="text-center">
          <p className="font-medium text-stone-900 dark:text-stone-100 font-mono">{formatCurrency(invoice.total)}</p>
