...
export function MetricCard({ title, value, subtitle, icon, variant = 'default', onClick }: MetricCardProps) {
    ...
    >
      <div className={`flex items-start gap-3 ${isRTL ? 'flex-row-reverse justify-between' : 'justify-between'}`}>
        <div className={`flex-1 min-w-0 ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          {subtitle && <p className="text-xs text-stone-500 dark:text-stone-400 mt-1 truncate">{subtitle}</p>}
        </div>
        {icon && <div className={`p-2 rounded-lg flex-shrink-0 ${styles.icon}`}>{icon}</div>}
      </div>
    </button>
  )
}
