      ...
      <div className="relative z-10 w-full max-w-lg rounded-xl bg-white dark:bg-stone-900 shadow-xl overflow-hidden flex flex-col max-h-[90vh]" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="flex items-center gap-4 p-4 bg-stone-50 dark:bg-stone-800/50 rounded-xl border border-stone-100 dark:border-stone-800">
            <div className="h-12 w-12 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-amber-700 dark:text-amber-400 font-bold text-lg">
                {subscription.customerName.charAt(0)}
            </div>
            <div className="flex-grow min-w-0">
              <p className="font-bold text-stone-900 dark:text-stone-100 truncate">{subscription.customerName}</p>
          ...
          {subscription.status === 'active' && !showChangePlan && !showCancelConfirm && (
            <div className="grid grid-cols-2 gap-3 pt-2">
                <button 
                    ...
                    className="flex flex-col items-center justify-center gap-2 p-4 rounded-xl border border-stone-200 dark:border-stone-800 hover:border-amber-300 dark:hover:border-amber-900/50 hover:bg-amber-50/30 transition-all group"
                >
                ...
                <button 
                    ...
                    className="flex flex-col items-center justify-center gap-2 p-4 rounded-xl border border-stone-200 dark:border-stone-800 hover:border-red-300 dark:hover:border-red-900/50 hover:bg-red-50/30 transition-all group"
                >
        ...
        <div className="p-5 bg-stone-50 dark:bg-stone-800/50 border-t border-stone-200 dark:border-stone-800 flex gap-3">
          {subscription.status !== 'active' ? (
            ...
            <button onClick={onClose} className="flex-1 px-4 py-2.5 text-sm font-bold text-stone-600 dark:text-stone-300 rounded-lg border border-stone-300 dark:border-stone-700 hover:bg-white transition-colors">
              {t('close')}
            </button>
          )}
        </div>
