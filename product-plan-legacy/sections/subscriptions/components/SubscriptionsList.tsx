...
import type { SubscriptionsListProps, PlanType } from '../types'
        ...
        <div>
          ...
          <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">
            ...
            {expiringCount > 0 && (
              <>
                {' · '}
                <span className="text-amber-600 dark:text-amber-400">{expiringCount} expiring soon</span>
              </>
            )}
          </p>
        </div>
      ...
      {filteredSubscriptions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-medium text-stone-900 dark:text-stone-100">
            {language === 'ar' ? 'لا توجد اشتراكات' : 'No subscriptions found'}
          </h3>
          <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">
            {statusFilter !== 'all' || typeFilter !== 'all'
              ? language === 'ar' ? 'جرب تغيير الفلاتر' : 'Try adjusting your filters'
              : language === 'ar' ? 'أضف أول اشتراك للبدء' : 'Create your first subscription to get started'}
          </p>
          {statusFilter === 'all' && typeFilter === 'all' && (
            <button
              onClick={onCreate}
          ...
          {filteredSubscriptions.map((subscription) => {
            ...
            return (
              <SubscriptionCard
                ...
                onDeactivate={() => onDeactivate?.(subscription.id)}
                language={language}
              />
            )
          })}
