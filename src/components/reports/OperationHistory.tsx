    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search operations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full py-2 text-sm bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 ${isRTL ? 'pe-4 ps-10 text-end' : 'ps-10 pe-4 text-start'}`} />
        </div>
        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value as OperationType | 'all')} className={`px-3 py-2 text-sm bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 ${isRTL ? 'text-end' : 'text-start'}`}>
          <option value="all">All Types</option>
          {Object.entries(operationConfig).map(([type, config]) => <option key={type} value={type}>{config.label}</option>)}
        </select>
      </div>

      {filteredOperations.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="p-4 bg-stone-100 dark:bg-stone-800 rounded-full mb-4"><History className="h-8 w-8 text-stone-400" /></div>
          <h3 className="text-lg font-medium text-stone-900 dark:text-stone-100">No operations found</h3>
          <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">{searchQuery || typeFilter !== 'all' ? 'Try adjusting your search or filters' : 'Operations will appear here as they happen'}</p>
        </div>
      ) : (
        <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden">
          <div className="divide-y divide-stone-100 dark:divide-stone-800">
            {filteredOperations.map((operation) => {
              ...
              return (
                <button key={operation.id} onClick={() => onOperationClick?.(operation.id)} className="w-full flex items-center gap-4 p-4 hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors text-left">
                  <div className={`p-2 rounded-lg flex-shrink-0 ${config.bg}`}><Icon className={`h-4 w-4 ${config.color}`} /></div>
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium text-stone-900 dark:text-stone-100">{operation.description}</p><p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">{config.label}</p></div>
                  <span className="text-xs text-stone-500 dark:text-stone-400 flex-shrink-0">{formatTime(operation.timestamp)}</span>
                </button>
              )
            })}
          </div>
