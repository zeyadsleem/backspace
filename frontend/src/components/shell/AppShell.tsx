...
export function AppShell({
	...
	return (
		...
		>
			{/* Mobile Header */}
			<header className="fixed top-0 right-0 left-0 z-50 flex h-14 items-center border-stone-200 border-b bg-white px-4 md:hidden dark:border-stone-800 dark:bg-stone-900">
				<button
        ...
        `}
			>
				...
				<button
					className={`absolute top-1/2 hidden h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-stone-200 bg-white transition-colors hover:bg-stone-100 md:flex dark:border-stone-800 dark:bg-stone-900 dark:hover:bg-stone-800 ${isRTL ? "-left-3" : "-right-3"}
          `}
			...
			{/* Main Content */}
			<main
				className={`flex h-screen flex-col pt-14 transition-all duration-300 md:pt-0 ${isRTL ? (sidebarCollapsed ? "md:mr-16" : "md:mr-60") : sidebarCollapsed ? "md:ml-16" : "md:ml-60"}
        `}
			>
